...
class EnhancedParkingDetector:
    def __init__(self, video_path: Optional[str] = None, image_path: Optional[str] = None) -> None:
        ...
        self.stable_occupied = [False] * len(self.posList)
        self.pending_occupied = [None] * len(self.posList)
        self.pending_frames = [0] * len(self.posList)
    ...
    def generate_parking_candidates(self, image: np.ndarray) -> List[Tuple[int, int]]:
        """Suggest slot origins from repeated parking-space line geometry."""
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 50, 150)
        horizontal_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (max(3, self.width // 2), 1))
        vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, max(3, self.height // 2)))
        ...
        parking_lines = cv2.bitwise_or(horizontal, vertical)
        candidates = []
        for y in range(0, image.shape[0] - self.height + 1, self.height):
            for x in range(0, image.shape[1] - self.width + 1, self.width):
                region = parking_lines[y : y + self.height, x : x + self.width]
                if cv2.countNonZero(region) >= max(20, (self.width + self.height) // 2):
    ...
    def check_parking_space(
        self, img_pro: np.ndarray, img: np.ndarray, occupied_flags: Optional[List[bool]] = None
    ) -> Tuple[int, int]:
        ...
        for index, pos in enumerate(self.posList):
            ...
            if occupied_flags is None:
                is_occupied = count >= config.OCCUPANCY_THRESHOLD
            else:
                is_occupied = occupied_flags[index]
        ...
        cvzone.putTextRect(
            img,
            f"LEFT/FREE: {space_counter}    OCCUPIED: {occupied_slots}    TOTAL: {len(self.posList)}",
            (100, 50),
            scale=2,
            thickness=3,
            offset=20,
    ...
    def get_video_occupancy(
        self, frame: np.ndarray, processed_img: np.ndarray
    ) -> List[bool]:
        """Match vehicles to slots, use local pixel evidence, and stabilize states."""
        raw_occupied = [False] * len(self.posList)
        results = self.car_detector.detect_cars(frame)

        for box in results.boxes:
            if int(box.cls) not in self.car_detector.car_classes:
                continue

            x1, y1, x2, y2 = map(int, box.xyxy[0])
            for index, (x, y) in enumerate(self.posList):
                margin_x = int(self.width * config.SLOT_INNER_MARGIN)
                margin_y = int(self.height * config.SLOT_INNER_MARGIN)
                left = x + margin_x
                top = y + margin_y
                right = x + self.width - margin_x
                bottom = y + self.height - margin_y
                intersection_width = max(0, min(x2, right) - max(x1, left))
                intersection_height = max(0, min(y2, bottom) - max(y1, top))
                intersection = intersection_width * intersection_height
                inner_area = max(1, (right - left) * (bottom - top))
                if intersection / inner_area >= config.VEHICLE_SLOT_OVERLAP:
                    raw_occupied[index] = True

        for index, (x, y) in enumerate(self.posList):
            slot = processed_img[y : y + self.height, x : x + self.width]
            pixel_occupied = cv2.countNonZero(slot) >= config.OCCUPANCY_THRESHOLD
            if pixel_occupied:
                raw_occupied[index] = True

        for index, current_state in enumerate(raw_occupied):
            if current_state == self.stable_occupied[index]:
                self.pending_occupied[index] = None
                self.pending_frames[index] = 0
            elif current_state == self.pending_occupied[index]:
                self.pending_frames[index] += 1
                if self.pending_frames[index] >= config.VIDEO_STABILITY_FRAMES:
                    self.stable_occupied[index] = current_state
                    self.pending_occupied[index] = None
                    self.pending_frames[index] = 0
            else:
                self.pending_occupied[index] = current_state
                self.pending_frames[index] = 1

        self.last_slot_status = self.stable_occupied.copy()
        return self.last_slot_status.copy()
    ...
    def process_video(self) -> None:
        ...
        if not self.posList:
            raise ValueError(
                "No parking spaces are saved. Run image mode first, select parking spaces, "
                "and press S to save them."
            )
        ...
        reference_image_path = self.image_path or config.DEFAULT_IMAGE_PATH
        reference_image = cv2.imread(reference_image_path)
        if reference_image is None:
            raise IOError(f"Could not load reference image: {reference_image_path}")
        ...
        original_positions = self.posList
        original_width, original_height = self.width, self.height
        self.posList = [
            (round(x * scale_x), round(y * scale_y)) for x, y in self.posList
        ]
        self.width = max(1, round(self.width * scale_x))
        self.height = max(1, round(self.height * scale_y))
        self.stable_occupied = [False] * len(self.posList)
        self.pending_occupied = [None] * len(self.posList)
        self.pending_frames = [0] * len(self.posList)
        logger.info(
            "Scaled parking layout from %sx%s to video resolution %sx%s",
            reference_width,
            reference_height,
            frame_width,
            frame_height,
        ...
        try:
            while True:
                ...
                occupied_flags = self.get_video_occupancy(img, processed_img)
                available_slots, occupied_slots = self.check_parking_space(
                    processed_img, img, occupied_flags
                )
            ...
            self.posList = original_positions
