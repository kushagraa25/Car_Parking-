...
class ParkingDetectionGUI(QMainWindow):
    ...
    def add_file_controls(self, layout):
        ...
        auto_btn = QPushButton("Suggest Parking Grid")
        auto_btn.clicked.connect(self.suggest_grid)
    ...
    def load_video(self):
        ...
        if file_name:
            ...
            if not ret:
                QMessageBox.critical(self, "Error", "Could not read the selected video.")
                return
            self.detector = EnhancedParkingDetector(video_path=file_name)
            self.detector.original_image = frame.copy()
            self.detector.current_image = frame.copy()
            self.update_frame()
            self.statusBar().showMessage("Video loaded. Review the first frame and suggest a grid.")
    ...
    def load_image(self):
        ...
        if file_name:
            ...
            self.detector.original_image = image.copy()
            self.detector.current_image = image.copy()
            self.update_frame()

    def suggest_grid(self):
        if not self.detector or self.detector.current_image is None:
            QMessageBox.information(self, "Choose a source", "Load an image or video first.")
            return
        candidates = self.detector.generate_parking_candidates(self.detector.current_image)
        self.detector.posList = candidates
        self.detector.is_reset = False
        self.update_frame()
        QMessageBox.information(
            self,
            "Grid suggestion ready",
            f"Suggested {len(candidates)} spaces. Remove incorrect road boxes, then save the layout.",
        )
    ...
    def update_frame(self):
        ...
        if self.video_path:
            occupied_flags = self.detector.get_video_occupancy(frame, processed_img)
            available_slots, occupied_slots = self.detector.check_parking_space(
                processed_img, frame, occupied_flags
            )
        else:
            available_slots, occupied_slots = self.detector.check_parking_space(
                processed_img, frame
            )
        self.detector.last_available_slots = available_slots
        self.detector.last_occupied_slots = occupied_slots
